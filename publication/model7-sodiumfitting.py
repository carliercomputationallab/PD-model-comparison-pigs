...
st = time.time()
#%%
def objective(x, Cp,Cd,  L,V, delP0, V_fill, Vr, predicted_cd ):
    '''The objective function needed to be minimised'''
    t = 240    
    predicted_cd = rk(t, x,  Cp,  L, V,  delP0, V_fill, Vr, predicted_cd )
...
def objective_fn(x,  Cp,  Cd,  L, V,  delP0, V_fill, Vr, predicted_cd ):
    '''The objective function needed to be minimised'''
...
#%%
#Runge-Kutta
def rk(t, x,  Cp, L, V, delP0, V_fill, Vr, predicted_cd):
    
    df_OV = pd.read_excel('fitted_X.xlsx').set_index('patient').iloc[:,:-1] # from model 7 results for all patients
    MTAC = df_OV.loc[patientlist[0]]
    
    PS_s = MTAC * 0.998 * abya0_s 
    PS_l = MTAC * 0.002 * abya0_l
    
    # change values of MTAC sodium only
    PS_s[2] = x * 0.998 * abya0_s[2]
    PS_l[2] = x * 0.002 * abya0_l[2]
...
def comdxdt(cd, t,  Cp,  L, V,  delP0, V_fill, Vr, PS_s, PS_l, x):
    ...
    J_vC = af*alpha[0]*LpS*(delP - sum(pr)) #ml/min
    J_vS = af*alpha[1]*LpS*(delP  - sigmas_pr) #ml/min
    J_vL = af*alpha[2]*LpS*(delP - sigmal_pr) #ml/min
...
LpS = 0.045 #after model 7- Vfitting.py
...
if __name__ == '__main__':
    ...
    MTAC = pd.DataFrame(columns = ['patient', 'MTAC-sodium', 'objective fn'], index = range(0, len(patientlist)))
    ...
    if value == 'y':
        for i, pfile in enumerate(patientlist):
            ...
            for var in range(10):
                ...
                x0 = random.random() #sodium PS
                              
                '''SLSQP optimisation'''
                result = scipy.optimize.minimize(objective_fn, x0, args = (Cp,  Cd,  L, V, delP0, V_fill, Vr, predicted_cd ),
...
#%%
            MTAC.loc[i, 'patient'] = pfile
            MTAC.loc[i, 'MTAC-sodium'] = optimised_values[obj_fn.index(min(obj_fn))]*0.998*abya0_s[2]+optimised_values[obj_fn.index(min(obj_fn))]*0.002*abya0_l[2]
            MTAC.loc[i, 'objective fn'] = min(obj_fn)
            #%%
            Cp, V, _,  L, delP0, V_fill, Vr, predicted_V, predicted_cd, Cd = input_values(file)
            predicted_cd = pd.DataFrame(columns= ["Urea", "Creatinine", "Sodium", "Phosphate", "Glucose", "Potassium"])
            predicted_cd.loc[0] = Cd.loc[0]
            predicted_cd= objective(MTAC.loc[0, 'MTAC-sodium'], Cp, Cd,  L, V, delP0, V_fill, Vr, predicted_cd )
            ...
            plt_text = 'MTAC='+str(MTAC.loc[0, 'MTAC-sodium'])+' ml/min/mmHg'
            ax[0,2].text(100, 130, plt_text)
            plt.legend()
            plt.suptitle('Sodium Fitting LpS 0.045')
            plt.show()                                                                 
