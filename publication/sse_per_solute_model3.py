...
if folder == '7':
    ...
    folder = '6_5ratio/'
sse_model3 = pd.DataFrame()
for file in map(os.path.basename,glob.glob('./spiderdata/'+folder+'m3_iteration*')):
    print(file)
    ...
    sse_model3 = pd.concat([sse_model3,sse_test_same.mean(axis = 1),sse_test_other.mean(axis = 1),sse_train.mean(axis = 1)], axis = 1)

keys = ['Test set-same session','SD1', 'Test set-other session','SD2', 'Training set', 'SD3']
result_m3 = pd.concat([sse_model3[0].mean(axis = 1),sse_model3[0].std(axis = 1),
                    sse_model3[1].mean(axis = 1),sse_model3[1].std(axis = 1),
                    sse_model3[2].mean(axis = 1),sse_model3[2].std(axis = 1)], axis = 1, 
                   keys = keys)
result_m3.to_csv('./spiderdata/'+folder+'model3_persolute_sse.csv')