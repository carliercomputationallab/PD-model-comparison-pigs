...
def compute(cd, t, x, predicted_cd, Cp, V, df_cd):
    ...
    MTAC = x[0:6]
    fct = x[6:12]
    SiCo = np.array([1] * len(solutes)) #SiCo
    ...
    # Villaroel-solute flux in hemodialysis and hemofiltration-after eq 23
...
if folder == '7':
    ...
    folder = '6_5ratio/'
sse_model2 = pd.DataFrame()
for file in map(os.path.basename,glob.glob('./spiderdata/'+folder+'m2_iteration*')):
    print(file)
    ...
    sse_model2 = pd.concat([sse_model2,sse_test_same.mean(axis = 1),sse_test_other.mean(axis = 1),sse_train.mean(axis = 1)], axis = 1)

keys = ['Test set-same session','SD1', 'Test set-other session','SD2', 'Training set', 'SD3']
result_m2 = pd.concat([sse_model2[0].mean(axis = 1),sse_model2[0].std(axis = 1),
                    sse_model2[1].mean(axis = 1),sse_model2[1].std(axis = 1),
                    sse_model2[2].mean(axis = 1),sse_model2[2].std(axis = 1)], axis = 1, 
                   keys = keys)
result_m2.to_csv('./spiderdata/'+folder+'model2_persolute_sse.csv')