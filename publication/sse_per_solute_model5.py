...
if folder == '7':
    ...
    folder = '6_5ratio/'
sse_model5 = pd.DataFrame()
for file in map(os.path.basename,glob.glob('./spiderdata/'+folder+'m5_iteration*')):
    print(file)
    ...
    sse_model5 = pd.concat([sse_model5,sse_test_same.mean(axis = 1),sse_test_other.mean(axis = 1),sse_train.mean(axis = 1)], axis = 1)

keys = ['Test set-same session','SD1', 'Test set-other session','SD2', 'Training set', 'SD3']
result_m5 = pd.concat([sse_model5[0].mean(axis = 1),sse_model5[0].std(axis = 1),
                    sse_model5[1].mean(axis = 1),sse_model5[1].std(axis = 1),
                    sse_model5[2].mean(axis = 1),sse_model5[2].std(axis = 1)], axis = 1, 
                   keys = keys)
result_m5.to_csv('./spiderdata/'+folder+'model5_persolute_sse.csv')
