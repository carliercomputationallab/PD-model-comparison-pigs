...
if folder == '7':
    ...
    folder = '6_5ratio/'
sse_model6 = pd.DataFrame()
for file in map(os.path.basename,glob.glob('./spiderdata/'+folder+'m6_iteration*')):
    print(file)
    ...
    sse_model6 = pd.concat([sse_model6,sse_test_same.mean(axis = 1),sse_test_other.mean(axis = 1),sse_train.mean(axis = 1)], axis = 1)

keys = ['Test set-same session','SD1', 'Test set-other session','SD2', 'Training set', 'SD3']
result_m6 = pd.concat([sse_model6[0].mean(axis = 1),sse_model6[0].std(axis = 1),
                    sse_model6[1].mean(axis = 1),sse_model6[1].std(axis = 1),
                    sse_model6[2].mean(axis = 1),sse_model6[2].std(axis = 1)], axis = 1, 
                   keys = keys)
result_m6.to_csv('./spiderdata/'+folder+'model6_persolute_sse.csv')