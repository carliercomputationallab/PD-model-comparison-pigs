...
if folder == '7':
    ...
    folder = '6_5ratio/'
for file in map(os.path.basename,glob.glob('./spiderdata/'+folder+'m1_iteration*')):
    print(file)
    ...
    sse_model1 = pd.concat([sse_model1,sse_test_same.mean(axis = 1),sse_test_other.mean(axis = 1),sse_train.mean(axis = 1)], axis = 1)

keys = ['Test set-same session','SD1', 'Test set-other session','SD2', 'Training set', 'SD3']
result_m1 = pd.concat([sse_model1[0].mean(axis = 1),sse_model1[0].std(axis = 1),
                    sse_model1[1].mean(axis = 1),sse_model1[1].std(axis = 1),
                    sse_model1[2].mean(axis = 1),sse_model1[2].std(axis = 1)], axis = 1, 
                   keys = keys)
# result_m1.to_csv('./spiderdata/'+folder+'model1_persolute_sse.csv')